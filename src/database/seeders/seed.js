...
const services = [
  { name: 'Corte Clásico', description: 'Corte clásico de cabello', price: 25, durationMin: 30, icon: 'corte', category: 'corte', order: 1 },
  { name: 'Corte + Barba', description: 'Corte de cabello con arreglo de barba', price: 40, durationMin: 45, icon: 'barba', category: 'barba', order: 2 },
  { name: 'Afeitado Clásico', description: 'Afeitado clásico con toalla caliente', price: 20, durationMin: 25, icon: 'barba', category: 'barba', order: 3 },
  { name: 'Coloración', description: 'Coloración de cabello profesional', price: 50, durationMin: 60, icon: 'color', category: 'color', order: 4 },
  { name: 'Diseño de Cejas', description: 'Diseño y arreglo de cejas', price: 15, durationMin: 15, icon: 'extra', category: 'extra', order: 5 },
  { name: 'Tratamiento Capilar', description: 'Tratamiento intensivo capilar', price: 35, durationMin: 40, icon: 'extra', category: 'extra', order: 6 }
];

const rewards = [
  { name: 'Corte Gratis', description: 'Gana un corte de cabello gratis', icon: 'corte', type: 'corte' },
  { name: '30% Descuento', description: 'Descuento del 30% en cualquier servicio', icon: 'descuento', type: 'descuento' },
  { name: 'Bebida Premium', description: 'Bebida premium gratuita', icon: 'bebida', type: 'bebida' },
  { name: 'Tratamiento Capilar', description: 'Tratamiento capilar gratuito', icon: 'tratamiento', type: 'tratamiento' },
...
export const runSeed = async () => {
  try {
    ...
    if (!existingClient) {
      const passwordHash = await bcrypt.hash(testClientUser.password, 10);
...
if (process.argv[1].endsWith('seed.js') || import.meta.url.endsWith('/seed.js')) {
  runSeed();
}
